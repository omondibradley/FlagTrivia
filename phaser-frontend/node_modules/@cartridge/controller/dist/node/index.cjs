...
var controllerWasm = require('@cartridge/controller-wasm');
var session = require('@cartridge/controller-wasm/session');
...
function toWasmPolicies(policies) {
  return [
    ...Object.entries(policies.contracts ?? {}).sort(([a], [b]) => a.toLowerCase().localeCompare(b.toLowerCase())).flatMap(
      ([target, { methods }]) => toArray(methods).slice().sort((a, b) => a.entrypoint.localeCompare(b.entrypoint)).map((m) => {
        if (m.entrypoint === "approve") {
          if ("spender" in m && "amount" in m && m.spender && m.amount) {
            const approvalPolicy = {
              target: starknet.getChecksumAddress(target),
              spender: m.spender,
          ...
          console.warn(
            `[DEPRECATED] Approve method without spender and amount fields will be rejected in future versions. Please update your preset or policies to include both 'spender' and 'amount' fields for approve calls on contract ${target}. Example: { entrypoint: "approve", spender: "0x...", amount: "0x..." }`
          );
        }
        return {
          target: starknet.getChecksumAddress(target),
          method: starknet.hash.getSelectorFromName(m.entrypoint),
    ...
    ...(policies.messages ?? []).map((p) => {
      ...
      (a, b) => a.scope_hash.toString().localeCompare(b.scope_hash.toString())
    )
  ];
}
...
var HeadlessAuthenticationError = class _HeadlessAuthenticationError extends Error {
  constructor(message, cause) {
    super(message);
    this.cause = cause;
    this.name = "HeadlessAuthenticationError";
    Object.setPrototypeOf(this, _HeadlessAuthenticationError.prototype);
  }
};
var InvalidCredentialsError = class _InvalidCredentialsError extends HeadlessAuthenticationError {
...
var HeadlessModeNotSupportedError = class _HeadlessModeNotSupportedError extends Error {
  constructor(operation) {
    super(`Operation "${operation}" is not supported in headless mode`);
    this.name = "HeadlessModeNotSupportedError";
    Object.setPrototypeOf(this, _HeadlessModeNotSupportedError.prototype);
  }
};
...
var SessionProvider = class extends BaseProvider {
  ...
  async connect() {
    ...
    if (sessionData) {
      ...
      const formattedPk = starknet.encode.addHexPrefix(publicKey);
      ...
      sessionRegistration.sessionKeyGuid = controllerWasm.signerToGuid({
...
exports.HeadlessAuthenticationError = HeadlessAuthenticationError;
exports.HeadlessModeNotSupportedError = HeadlessModeNotSupportedError;
