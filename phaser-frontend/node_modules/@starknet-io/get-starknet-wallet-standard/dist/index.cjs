...
const features$1 = require('./features.cjs');
require('ox');
...
class StarknetInjectedWallet {
  ...
  get chains() {
    return chains.WELL_KNOWN_STARKNET_CHAINS.slice();
  }
  get accounts() {
    if (__privateGet(this, _account)) {
      return [
...
onNetworkChanged_fn = function(chainId, accounts) {
  ...
  const chain = chains.formatStarknetChainId(chainId);
  if (!chains.isStarknetChain(chain)) {
...
exports.StarknetInjectedWallet = StarknetInjectedWallet;
